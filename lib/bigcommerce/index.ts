...
const channelIdSegment = isDemoMode
  ? ''
...
const domain = isDemoMode
  ? ''
  : `https://store-${process.env.BIGCOMMERCE_STORE_HASH!}${channelIdSegment}`;
const endpoint = isDemoMode ? '' : `${domain}.${BIGCOMMERCE_GRAPHQL_API_ENDPOINT}`;
